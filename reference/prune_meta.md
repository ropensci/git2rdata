...
Removes all **valid** metadata (`.yml` files) from the `path` when they
don't have accompanying data (`.tsv` file). **Invalid** metadata
triggers a warning without removing the metadata file.

Use this function with caution since it will remove all valid metadata
files without asking for confirmation. We strongly recommend to use this
...
#> [fbd68c0] 2025-12-10: add iris data
status(repo)
