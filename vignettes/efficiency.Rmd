...
rs <- lapply(
  ...
  function(x) {
    ...
    data.frame(commit = seq_along(repo_size[x, ]), size = repo_size[x, ], 
               ...
               fun = fun, method = method,stringsAsFactors = FALSE)
  }
)
rs <- do.call(rbind, rs)
ggplot(rs, aes(x = commit, y = size / 2^10, colour = fun, linetype = method)) +
  geom_line() +
...
ggplot(rs, aes(x = commit, y = rel_size, colour = fun, linetype = method)) +
  geom_line() +
