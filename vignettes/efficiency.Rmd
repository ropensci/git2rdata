...
We start by writing the dataset as is with `write.table()`, `saveRDS()`, `write_vc()` and `write_vc()` without storage optimization. Note that `write_vc()` uses optimization by default. Since `write_vc()` creates two files for each data set, we take their combinated file size into account.
...
The reduction in file size when storing in factors depends on the length of the labels, the number of levels and the number of observations. The figure below illustrates the huge gain as soon as the level labels contain a few characters. The gain is less pronounces when the factor has a large number of levels. The optimization fails only in the extreme cases with very short factor labels and a high number of labels.

```{r factor_label_length, echo = FALSE, fig.cap = "Effect of the label length on the efficiency of storing factor optimized, assuming 1000 observations. Black line: 1000 levels, blue line: 10 levels, red line: 3 levels."}
...
```{r factor_observations, echo = FALSE, fig.cap = "Effect of the number of observations on the efficiency of storing factor optimized assuming labels with 10 characters. Black line: 1000 levels, blue line: 10 levels, red line: 3 levels."}
...
lines(repo_size["write_vc.optimized", ], col = "green")
lines(repo_size["write_vc.verbose", ], col = "blue")
```

```{r plot_rel_git_size, echo = FALSE, fig.cap = "Relative size of the git repository when compared to write.table(). black: saveRDS(), solid red: write.table(), dashed red: sorted write.table() blue: write_vc(), verbose, green: write_vc(), optimized."}
plot(repo_size["saveRDS", ] / repo_size["write.table.sorted", ], type = "l", 
     ...
     ylim = c(0.5, 7))
lines(repo_size["write_vc.optimized", ] / repo_size["write.table.sorted", ], 
      col = "green")
...
boxplot(time ~ expr, data = mb, 
        ylim = c(0, max(mb$time)), ylab = "Time (in milliseconds)")
```
