...
```{r plot_file_timings, echo = FALSE, fig.cap = "Density of the write timings for the different methods. black: saveRDS(), red: write.table(), green: write_vc(), optimised, blue: write_vc(), verbose."}
...
mb <- microbenchmark(
  ...
  read_vc.optim = read_vc("airbag_optimise", root),
...
The timings on reading the data is is more extreme story. Reading the binary format takes about 8% of the time needed to read the standard plain text format using `read.table()`. `read_vc()` takes about 70% of the time needed by `read.table()`, which at first seems strange because `read_vc()` calls `read.table()` to read the files and has some extra work to convert the variables to the correct data type. The main difference is that `read_vc()` knows the required data type _a priori_ and passes this info to `read.table()`. Otherwise, `read.table()` has to guess the correct data type from the file.

```{r plot_read_timings, echo = FALSE, fig.cap = "Density of the read timings for the different methods. black: readRDS(), red: read.table(), green: read_vc(), optimised, blue: read_vc(), verbose."}
plot(density(mb$time[mb$expr == "readRDS"]), xlim = c(0, 250), 
     xlab = "Time (in milliseconds)", ylab = "Density", 
     main = "Density of timings")
lines(density(mb$time[mb$expr == "read.table"]), col = "red")
lines(density(mb$time[mb$expr == "read_vc.optim"]), col = "green")
lines(density(mb$time[mb$expr == "read_vc.verbose"]), col = "blue")
```
