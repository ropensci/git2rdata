...
theme_inbo <- function(base_size = 12, base_family = "") {
  ...
  theme(
    ...
    legend.key = element_rect(fill = "#F3F3F3", colour = NA),
...
tmp_repo <- function() {
  ...
  dir.create(root)
  repo <- init(root) # nolint
  config(repo, user.name = "me", user.email = "me@me.com") # nolint
  return(repo)
}
...
ggplot(
  rs,
  aes(x = commit, y = size / 2^10, colour = fun, linetype = optimized)
) +
  geom_line() +
