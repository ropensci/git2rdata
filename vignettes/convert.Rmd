...
This is useful when you want to store data in a different format than you use it in R, or when you need to apply reversible transformations for storage optimization or compatibility.
...
raw_content <- readLines(file.path(root, "people.csv"))
cat(raw_content, sep = "\n")
```
...
- **Package availability**: All packages referenced in the `convert` argument must be available when calling `write_vc()`. The function checks for package availability at write time.
...
- The `convert` argument only accepts functions in the `package::function` format. Anonymous functions or functions from the global environment are not supported.

- Conversions must be reversible. The `read` function should be able to restore the original data from the converted form.
