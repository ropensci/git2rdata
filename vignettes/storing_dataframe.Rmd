...
```{r}
library(git2rdata)
write_vc(x = x, file = "unsorted", root = path)
```
...
The new version of the dataframe is compared with the old metadata. Any mismatch in metadata leads to an error. Use `strict = FALSE` to turn this error into an warning.
...
1. use `prune_meta()` to remove any unused metadata files.
...
status(repo)
# step 3
prune_meta(root = repo, path = subdir, stage = TRUE)
status(repo)
# step 4
