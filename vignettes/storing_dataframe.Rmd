...
1. use `prune_tsv()` to remove and stage all existing data files (`.tsv`). Note that the metadata is unchanged.
...
1. use `prune_yml()` to remove any unused metadata files.
...
status(repo)
# step 1
prune_tsv(root = repo, path = subdir, stage = TRUE)
status(repo)
#step 2
for (group in groups) {
  i <- sample(nrow(x), 3)
...
status(repo)
# step 3
prune_yml(root = repo, path = subdir, stage = TRUE)
status(repo)
# step 4
